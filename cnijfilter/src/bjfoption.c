...
short SetCmdOption(int, char **, LPBJF_OPTINFO, LPBJFLTCOLORSYSTEM,
                   LPBJFLTDEVICE, LPCNCLPAPERSIZE, char *, char *, char *);
static void usage(char *);
static void init_optioninfo(LPBJF_OPTINFO lpbjfoption);
static int cmdlinesw(poptContext *, OPT *, LPBJF_OPTINFO, LPBJFLTCOLORSYSTEM,
                     LPBJFLTDEVICE, LPCNCLPAPERSIZE, char *, char *, char *,
                     char *);
...
const char usage_str_ime[] =
    "       switches: [ --imageres     1 - 32767               ]\n";
const char usage_str_car[] =
    "                 [ --cartridge    cartridgetype           ]\n";
const char usage_str_med[] =
    "                 [ --media        mediatype               ]\n";
const char usage_str_hal[] =
    "                 [ --halftoning   halftonetype            ]\n";
const char usage_str_qua[] =
    "                 [ --quality      1 - 5                   ]\n";
const char usage_str_gra[] =
    "                 [ --grayscale                            ]\n";
const char usage_str_pap[] =
    "                 [ --papersize    size                    ]\n";
const char usage_str_pal[] =
    "                 [ --paperload    position                ]\n";
const char usage_str_bor[] =
    "                 [ --borderless                           ]\n";
const char usage_str_ext[] =
    "                 [ --extension    0 - 3                   ]\n";
const char usage_str_loc[] =
    "                 [ --location     position                ]\n";
const char usage_str_fit[] =
    "                 [ --fit                                  ]\n";
const char usage_str_ful[] =
    "                 [ --full                                 ]\n";
const char usage_str_per[] =
    "                 [ --percent      20 - 400                ]\n";
const char usage_str_cop[] =
    "                 [ --copies       1 - 999                 ]\n";
const char usage_str_int[] =
    "                 [ --renderintent intent                  ]\n";
const char usage_str_gam[] =
    "                 [ --gamma        1.4 / 1.8 /2.2          ]\n";
const char usage_str_bac[] =
    "                 [ --balance_c    -50  -   50             ]\n";
const char usage_str_bam[] =
    "                 [ --balance_m    -50  -   50             ]\n";
const char usage_str_bay[] =
    "                 [ --balance_y    -50  -   50             ]\n";
const char usage_str_bak[] =
    "                 [ --balance_k    -50  -   50             ]\n";
const char usage_str_tone[] =
    "                 [ --tone         -50  -   50             ]\n";
const char usage_str_contrast[] =
    "                 [ --contrast     -50  -   50             ]\n";
const char usage_str_den[] =
    "                 [ --density      -50  -   50             ]\n";
const char usage_str_gap[] =
    "                 [ --papergap     gaptype                 ]\n";
const char usage_str_bbox[] =
    "                 [ --bbox         left,bottom,right,top   ]\n";
const char usage_str_duplex[] =
    "                 [ --duplex                               ]\n";
const char usage_str_staple[] =
    "                 [ --stapleside   longside / shortside    ]\n";
const char usage_str_ics[] =
...
void usage(char *modelname) {
  ...
  snprintf(useagebjfilter, sizeof(useagebjfilter), "%s%s%s", usage_bjfilter,
           small_tmpname, usage_switches);

  fprintf(
      stderr,
      "\n%s\n%s\n%s\n\n%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%"
      "s%s%s\n",
      VERSION_STR, COPYRIGHT_STR0, COPYRIGHT_STR1, useagebjfilter,
      usage_str_ime, usage_str_car, usage_str_med,
      ((formattype < CND_FORMATTYPE_12) ? usage_str_hal : ""), usage_str_qua,
      usage_str_gra, usage_str_pap, usage_str_pal,
      (borderless_support ? usage_str_bor : ""),
      (borderless_support ? usage_str_ext : ""), usage_str_loc, usage_str_fit,
      usage_str_ful, usage_str_per, usage_str_cop, usage_str_int, usage_str_gam,
      usage_str_bac, usage_str_bam, usage_str_bay,
      (tone_support ? usage_str_tone : ""),
      ((formattype >= CND_FORMATTYPE_6) ? "" : usage_str_bak), /* Ver.2.70 */
      ((formattype >= CND_FORMATTYPE_6) ? usage_str_contrast
                                        : ""), /* Ver.2.70 */
      usage_str_den, (papergap_support ? usage_str_gap : ""),
      (duplex_support ? usage_str_duplex : ""),
      (duplex_support ? usage_str_staple : ""),
      (ics_support ? usage_str_ics : ""), usage_str_bbox);
}
...
static short GetPaperSize(short id, long *width, long *height) {
  ...
  while ((id != PaperTbl[paper][PSID]) && (PaperTbl[paper][PSID] != -1))
    paper++;

  if (PaperTbl[paper][PSID] == -1)
    goto onErr;
...
/*-------------------------------------------------------------*/
short SetDisableFlag(LPCNCLDB lpdbTop, short dbsize, short id, short value,
                     short flag) {
  LPCNCLDB lpdb = lpdbTop;
  short ret = -1;
  short i;
...
/*-------------------------------------------------------------*/
short SetTemporaryFlag(LPCNCLDB lpdbTop, short dbsize, short id, short value,
                       short flag) {
  LPCNCLDB lpdb = lpdbTop;
  short ret = -1;
  short i;
...
/*-------------------------------------------------------------*/
short SetCmdOption(int cargc, char *cargv[], LPBJF_OPTINFO lpbjfoption,
                   LPBJFLTCOLORSYSTEM bjcolor, LPBJFLTDEVICE bjdevice,
                   LPCNCLPAPERSIZE psize, char *dispname, char *filename,
                   char *modelname) {
  OPT opt;
  poptContext optcon;
  char confname[256];
  ...
  struct poptOption optionsTable[] = {
      ...
      /*   to support 1.) add an option --imageres, to cover with the bug in the
         bmp filter in           */
      /*                  previous version of ghostscript. (lacking image
         resolution information)        */
      /*              2.) add --display, to specify X server to show
         printui/stamon                      */
      ...
      {OPTSTRINKCARTRIDGESETTINGS, 0, POPT_ARG_STRING,
       &opt.inkcartridgesettings, OPTINKCARTRIDGESETTINGS},
      ...
      {OPTSTRSUPPORTNETWORK, 0, POPT_ARG_NONE, &opt.supportnetwork,
       OPTSUPPORTNETWORK},
      {NULL, 0, 0, NULL, 0}};
  ...
  MakeModelnameConfname(cargv[0], modelname, confname, BJFILTERXXXXRCPATH,
                        BJFILTERDOTCONF);
  first_modelstrnum = strlen(modelname);

  if ((ret = cmdlinesw(&optcon, &opt, lpbjfoption, bjcolor, bjdevice, psize,
                       dispname, filename, confname, modelname)) < 0) {
...
/*-------------------------------------------------------------*/
int cmdlinesw(poptContext *optcon, OPT *opt, LPBJF_OPTINFO lpbjf_optinfo,
              LPBJFLTCOLORSYSTEM bjcolor, LPBJFLTDEVICE bjdevice,
              LPCNCLPAPERSIZE psize, char *dispname, char *fname,
              char *confname, char *modelname) {
  int rc, setopt[4];
  int id;
  UIDB uidb;
  ...
  /* analize commandline arguments with popt, then set option values into struct
   * opt.*/
  ...
  if (setopt[OPTINDEX(OPTSUPPORTNETWORK)] & OPTBIT(OPTSUPPORTNETWORK)) {
    if (bjf_get_resource_id(confname, OPTSTRSUPPORTNETWORK, modelname) ==
        BJFRCACCESSERROR) {
      fprintf(stderr, "%d\n", NETWORK_SUPPORT_NO);
    } else {
      fprintf(stderr, "%d\n", NETWORK_SUPPORT_YES);
  ...
  if (poptPeekArg(*optcon) == 0) {
    ...
    *fname = '\0';
    ...
    /* filename should not be restricted to one.
             so I should consider the way to pass the filename string array,
       later.*/
  ...
  if ((id = bjf_get_resource_id(confname, "model", modelname)) ==
      BJFRCACCESSERROR) {
  ...
  /* Database access Init code */
  {
    ...
    if (CNCL_GetDefaults(&uidb.nominfo, (void *)bjlibdir, uidb.lpdbTop,
                         uidb.dbsize) < 0) {
      fprintf(stderr, "Error: CNCL_GetDefaults\n");
      goto onError;
    }
    if (CNCL_InitMenulink(&uidb.nominfo, (void *)bjlibdir, uidb.lpdbTop,
                          uidb.dbsize) < 0) {
    ...
    if ((formattype =
             CNCL_GetTableFormatType(&uidb.nominfo, (void *)bjlibdir)) < 0) {
    ...
    if (formattype >= CND_FORMATTYPE_6) {
      if ((DefaultIntent =
               GetCurrentnValue(uidb.lpdbTop, uidb.dbsize, CNCL_INTENT)) >= 0) {
    ...
    if ((DefaultGamma = GetCurrentnValue(uidb.lpdbTop, uidb.dbsize,
                                         CNCL_INPUT_GAMMA)) > 0) {
  ...
  /* Check whether following options(or parameters) are supported or not.
    ...
    "renderintent vivid"--
  */
  if (FindValue(uidb.lpdbTop, uidb.dbsize, CNCL_MARGINTYPE, CND_MARGIN_MINUS) >=
      0) {
    borderless_support = 1;
  }

  if (FindValue(uidb.lpdbTop, uidb.dbsize, CNCL_PAPERGAP_COMMAND,
                CND_PGAP_CMD_AUTO) >= 0) {
    papergap_support = 1;
  }

  if (FindValue(uidb.lpdbTop, uidb.dbsize, CNCL_DUPLEX_PRINTING,
                CND_DUPLEX_AUTO) >= 0) {
    duplex_support = 1;
  }

  ics_support = 1;
  if (GetCurrentnValue(uidb.lpdbTop, uidb.dbsize, CNCL_INKCARTRIDGESETTINGS) <
      0) {
    ics_support = 0;
  } else if (FindValue(uidb.lpdbTop, uidb.dbsize, CNCL_INKCARTRIDGESETTINGS,
                       CND_CARTRIDGE_NA) >= 0) {
    ics_support = 0;
  }

  if (FindValue(uidb.lpdbTop, uidb.dbsize, CNCL_VIVID, CND_VIVID_SUPPORT) >=
      0) { /* Ver.2.70 */
    vivid_support = 1;
  }

  if (FindValue(uidb.lpdbTop, uidb.dbsize, CNCL_BLACK_ADJUSTMENT,
                CND_BLACK_ADJUSTMENT) >= 0) {
    tone_support = 1;
  }
  ...
  if (count_switch > 1) {
    fprintf(stderr,
            "Error: Only one of the following options can be selected.\n");
  ...
  if (setopt[OPTINDEX(OPTCARTRIDGE)] & OPTBIT(OPTCARTRIDGE)) {
    ...
    if ((id = bjf_get_resource_id(confname, OPTSTRCARTRIDGE, opt->cartridge)) ==
        BJFRCACCESSERROR) {
    ...
    if (QueryValue(uidb.lpdbTop, uidb.dbsize, CNCL_CARTRIDGE, id) >= 0) {
      ...
      CNCL_GetMenulink(&uidb.nominfo, (void *)bjlibdir, uidb.lpdbTop,
                       uidb.dbsize);
    } else {
  ...
  if (setopt[OPTINDEX(OPTMEDIA)] & OPTBIT(OPTMEDIA)) {
    ...
    if ((id = bjf_get_resource_id(confname, OPTSTRMEDIA, opt->media)) ==
        BJFRCACCESSERROR) {
  ...
  if (setopt[OPTINDEX(OPTGRAY)] & OPTBIT(OPTGRAY)) {
    if (QueryValue(uidb.lpdbTop, uidb.dbsize, CNCL_GRAYSCALE,
                   CND_BJGRAYSCALE_ON) >= 0) {
      SetTemporaryFlag(uidb.lpdbTop, uidb.dbsize, CNCL_GRAYSCALE,
                       CND_BJGRAYSCALE_ON, 1);
      CNCL_GetMenulink(&uidb.nominfo, (void *)bjlibdir, uidb.lpdbTop,
                       uidb.dbsize);
  ...
  if (setopt[OPTINDEX(OPTBORDERLESS)] & OPTBIT(OPTBORDERLESS)) {
    if (borderless_support) {
      if (QueryValue(uidb.lpdbTop, uidb.dbsize, CNCL_MARGINTYPE,
                     CND_MARGIN_MINUS) >= 0) {
      ...
      fprintf(
          stderr,
          "Error: \"--borderless\" option is not supported by this printer\n");
      /* inappropriate selection error. */
      goto onError;
    }
  }
  ...
  if (setopt[OPTINDEX(OPTEXTENSION)] & OPTBIT(OPTEXTENSION)) {
    if (!(setopt[OPTINDEX(OPTBORDERLESS)] & OPTBIT(OPTBORDERLESS))) {
      fprintf(stderr, "Error: Cannot select \"--extension\" option without "
                      "\"--borderless\" \n");
      goto onError;
    }
    ...
    if (formattype == CND_FORMATTYPE_4) { /* DB supports extension_level info */
      ...
      if (QueryValue(uidb.lpdbTop, uidb.dbsize, CNCL_OVERMARGIN_LEVEL,
                     opt->extension) >= 0) {
    ...
    if (ext < 0) { /* default value of EXTENSION not found */
      if ((id = bjf_get_resource_id(confname, "extension_default",
                                    modelname)) == BJFRCACCESSERROR) {
  ...
  if (setopt[OPTINDEX(OPTQUALITY)] & OPTBIT(OPTQUALITY)) {
    ...
    // high quality
    if ((opt_quality_max = bjf_get_resource_id(confname, OPTSTRQUALITY, "1")) ==
        BJFRCACCESSERROR) {
    ...
    // high speed
    if ((opt_quality_min = bjf_get_resource_id(confname, OPTSTRQUALITY, "5")) ==
        BJFRCACCESSERROR) {
      fprintf(stderr, "Error: invalid print quality\n");
      ret = OPT_ERR_QUA - 50;
      goto onErrorMessage;
    ...
    // specified quality
    if ((opt_quality_id = bjf_get_resource_id(
             confname, OPTSTRQUALITY, opt->quality)) == BJFRCACCESSERROR) {
      fprintf(stderr, "Error: invalid print quality\n");
      ret = OPT_ERR_QUA - 50;
      goto onErrorMessage;
    }
    ...
    // default quality
    def_quality_id =
        GetCurrentnValue(uidb.lpdbTop, uidb.dbsize, CNCL_PRINTQUALITY);
    ...
    for (quality_idx_span = 0; quality_idx_span < 5; quality_idx_span++) {
      ...
      // high priority
      cur_quality_id =
          opt_quality_id + (quality_idx_span * opt_quality_direction);
      if (QueryValue(uidb.lpdbTop, uidb.dbsize, CNCL_PRINTQUALITY,
                     cur_quality_id) >= 0) {
        SetTemporaryFlag(uidb.lpdbTop, uidb.dbsize, CNCL_PRINTQUALITY,
                         cur_quality_id, 1);
        CNCL_GetMenulink(&uidb.nominfo, (void *)bjlibdir, uidb.lpdbTop,
                         uidb.dbsize);
  ...
  if (setopt[OPTINDEX(OPTLOAD)] & OPTBIT(OPTLOAD)) {
    ...
    if ((id = bjf_get_resource_id(confname, OPTSTRLOAD, opt->paperload)) ==
        BJFRCACCESSERROR) {
    ...
    if (id == CND_SUPPLY_FRONT_FOR_PLAIN) {
      ...
      if (FindValue(uidb.lpdbTop, uidb.dbsize, CNCL_MEDIASUPPLY,
                    CND_SUPPLY_CASSETTE_04) < 0) {
  ...
  if (setopt[OPTINDEX(OPTPSIZE)] & OPTBIT(OPTPSIZE)) {
    if ((id = bjf_get_resource_id(confname, OPTSTRPSIZE, opt->papersize)) ==
        BJFRCACCESSERROR) {
  ...
  if (setopt[OPTINDEX(OPTDUPLEX)] & OPTBIT(OPTDUPLEX)) {
    if (duplex_support) {
      if (QueryValue(uidb.lpdbTop, uidb.dbsize, CNCL_DUPLEX_PRINTING,
                     CND_DUPLEX_AUTO) >= 0) {
      ...
      fprintf(stderr,
              "Error: \"--duplex\" option is not supported by this printer\n");
      /* inappropriate selection error. */
      goto onError;
    }
  }
  ...
  if (setopt[OPTINDEX(OPTSTAPLESIDE)] & OPTBIT(OPTSTAPLESIDE)) {
    if (!(setopt[OPTINDEX(OPTDUPLEX)] & OPTBIT(OPTDUPLEX))) {
      fprintf(stderr, "Error: Cannot select \"--stapleside\" option without "
                      "\"--duplex\" \n");
      goto onError;
    }
  ...
  if (setopt[OPTINDEX(OPTHALFTONE)] & OPTBIT(OPTHALFTONE)) {
    ...
    if (formattype >= CND_FORMATTYPE_12) {
      fprintf(
          stderr,
          "Error: \"--halftoning\" option is not supported by this printer\n");
      /* inappropriate selection error. */
      goto onError;
    }

    if ((id = bjf_get_resource_id(confname, OPTSTRHALFTONE, opt->halftoning)) ==
        BJFRCACCESSERROR) {
  ...
  if (setopt[OPTINDEX(OPTPAPERGAP)] & OPTBIT(OPTPAPERGAP)) {
    ...
    if (papergap_support) {
      if ((id = bjf_get_resource_id(confname, OPTSTRPAPERGAP, opt->papergap)) ==
          BJFRCACCESSERROR) {
      ...
      if (QueryValue(uidb.lpdbTop, uidb.dbsize, CNCL_PAPERGAP_COMMAND, id) >=
          0) {
        SetTemporaryFlag(uidb.lpdbTop, uidb.dbsize, CNCL_PAPERGAP_COMMAND, id,
                         1);
        CNCL_GetMenulink(&uidb.nominfo, (void *)bjlibdir, uidb.lpdbTop,
                         uidb.dbsize);
      } else {
      ...
      fprintf(
          stderr,
          "Error: \"--papergap\" option is not supported by this printer\n");
      /* inappropriate selection error. */
      goto onError;
    }
  }
  ...
  if (setopt[OPTINDEX(OPTRENDER)] & OPTBIT(OPTRENDER)) {
    ...
    if ((id = bjf_get_resource_id(confname, OPTSTRRENDER, opt->renderintent)) ==
        BJFRCACCESSERROR) {
  ...
  if (setopt[OPTINDEX(OPTGAMMA)] & OPTBIT(OPTGAMMA)) {
    ...
    if ((id = bjf_get_resource_id(confname, OPTSTRGAMMA, opt->gamma)) ==
        BJFRCACCESSERROR) {
  ...
  if (setopt[OPTINDEX(OPTBALK)] & OPTBIT(OPTBALK)) {
    if (formattype >= CND_FORMATTYPE_6) { /* not support balance_k */
      fprintf(
          stderr,
          "Error: \"--balance_k\" option is not supported by this printer\n");
      /* inappropriate selection error. */
      goto onError;
  ...
  if (setopt[OPTINDEX(OPTTONE)] & OPTBIT(OPTTONE)) {
    if (formattype < CND_FORMATTYPE_11 ||
        !tone_support) { /* not support tone */
      fprintf(stderr,
              "Error: \"--tone\" option is not supported by this printer\n");
      /* inappropriate selection error. */
      goto onError;

    } else if (CNCL_ConfirmBlackAdjMedia(&uidb.nominfo, (void *)bjlibdir,
                                         uidb.lpdbTop, uidb.dbsize) <=
               0) { /* invalid media */
  ...
  if (setopt[OPTINDEX(OPTCONTRAST)] & OPTBIT(OPTCONTRAST)) {
    if (formattype >= CND_FORMATTYPE_6) { /* support contrast */
      ...
      fprintf(
          stderr,
          "Error: \"--contrast\" option is not supported by this printer\n");
      /* inappropriate selection error. */
      goto onError;
    }
  }
  ...
  bjdevice->bjfltPaperSize =
      GetCurrentnValue(uidb.lpdbTop, uidb.dbsize, CNCL_PAPERSIZE);
  if (bjdevice->bjfltPaperSize == CND_SIZE_USER) {
    ...
    if (psize->nSelPaperWidth == -1 || psize->nSelPaperLength == -1) {
      ...
      fprintf(
          stderr,
          "       both \"--paperwidth\" and \"--paperheight\" must be set.\n");
      goto onError;
    }
  } else {
    ...
    if ((setopt[OPTINDEX(OPTPAPERWIDTH)] & OPTBIT(OPTPAPERWIDTH)) ||
      ...
      fprintf(stderr, "Error: Cannot select \"--paperwidth\" and "
                      "\"--paperheight\" without \"--papersize user\" \n");
      goto onError;
    }

    if ((ret =
             GetPaperSize(bjdevice->bjfltPaperSize, &bjdevice->bjfltPaperWidth,
                          &bjdevice->bjfltPaperHeight)) < 0)
      goto onError;
  }

  if ((ret = CNCL_GetMargin(&uidb.nominfo, (void *)bjlibdir, uidb.lpdbTop,
                            psize, uidb.dbsize)) < 0) {
    if (ret < -2) { /* height out of range. */
      fprintf(stderr,
              "Error: invalid paperheight value. (integer: %5ld - %5ld "
              "[1/100mm])\n",
    ...
    if (ret == -2) { /* width out of range. */
      fprintf(stderr,
              "Error: invalid paperwidth value.  (integer: %5ld - %5ld "
              "[1/100mm])\n",
  ...
  if (setopt[OPTINDEX(OPTINKCARTRIDGESETTINGS)] &
      OPTBIT(OPTINKCARTRIDGESETTINGS)) {
    if (ics_support) {
      if ((id = bjf_get_resource_id(confname, OPTSTRCARTRIDGE,
                                    opt->inkcartridgesettings)) ==
          BJFRCACCESSERROR) {
      ...
      if (QueryValue(uidb.lpdbTop, uidb.dbsize, CNCL_INKCARTRIDGESETTINGS,
                     id) >= 0) {
        SetTemporaryFlag(uidb.lpdbTop, uidb.dbsize, CNCL_INKCARTRIDGESETTINGS,
                         id, 1);
        CNCL_GetMenulink(&uidb.nominfo, (void *)bjlibdir, uidb.lpdbTop,
                         uidb.dbsize);
      } else {
        fprintf(stderr,
                "Error: inappropriate inkcartridgesettings selection\n");
        /* inappropriate selection error. */
        goto onError;
      }

    } else { /* not support inkcartridgesettings */
      fprintf(stderr, "Error: \"--inkcartridgesettings\" option is not "
  ...
  /* Rotate (Ver.3.80) */
  if (FindValue(uidb.lpdbTop, uidb.dbsize, CNCL_IMG_DIRECTION_CMD,
                CND_IMG_DIRECTION_CMD_TYPEA) >= 0) {
  ...
  if (formattype >= CND_FORMATTYPE_9) {
    if (FindValue(uidb.lpdbTop, uidb.dbsize, CNCL_COMMAND_TYPE,
                  CND_COMMAND_TYPE_01) >= 0) {
  ...
  if (formattype >= CND_FORMATTYPE_8) {
    ...
    if ((confirm_return_code =
             CNCL_ConfirmSupplySize(&uidb.nominfo, (void *)bjlibdir,
                                    uidb.lpdbTop, uidb.dbsize)) == 1) {
    ...
    if (duplex_support) {
      if ((confirm_return_code =
               CNCL_ConfirmSizeDuplex(&uidb.nominfo, (void *)bjlibdir,
                                      uidb.lpdbTop, uidb.dbsize)) == 1) {
  ...
  bjdevice->bjfltMediaType =
      GetCurrentnValue(uidb.lpdbTop, uidb.dbsize, CNCL_MEDIATYPE);
  bjdevice->bjfltPrintQuality =
      GetCurrentnValue(uidb.lpdbTop, uidb.dbsize, CNCL_PRINTQUALITY);
  bjdevice->bjfltMediaSupply =
      GetCurrentnValue(uidb.lpdbTop, uidb.dbsize, CNCL_MEDIASUPPLY);

  /* Convert "frontplain" to "asf" or "front" */
  if (change_paperload) {
    if ((bjdevice->bjfltMediaType == CND_MEDIA_PLAIN) &&
        ((bjdevice->bjfltPaperSize == CND_SIZE_A4) ||
         (bjdevice->bjfltPaperSize == CND_SIZE_LETTER) ||
         (bjdevice->bjfltPaperSize == CND_SIZE_B5))) {
      bjdevice->bjfltMediaSupply = CND_SUPPLY_CASSETTE_04;
    }
  }

  bjdevice->bjfltPaperGap =
      GetCurrentnValue(uidb.lpdbTop, uidb.dbsize, CNCL_PAPERGAP_COMMAND);
  if (bjdevice->bjfltPaperGap == -1)
    bjdevice->bjfltPaperGap = CND_PGAP_CMD_NA;

  /* bjdevice->bjfltPaperSize is set already. */
  /*bjdevice->bjfltPaperSize		= GetCurrentnValue(uidb.lpdbTop,
   * uidb.dbsize, CNCL_PAPERSIZE);*/

  bjdevice->bjfltBinMethod =
      GetCurrentnValue(uidb.lpdbTop, uidb.dbsize, CNCL_DITHER_PAT);
  if (bjdevice->bjfltBinMethod == CND_UIBIN_PATTERN_HS)
    bjdevice->bjfltBinMethod = CND_UIBIN_PATTERN;

  bjdevice->bjfltBanner =
      GetCurrentnValue(uidb.lpdbTop, uidb.dbsize, CNCL_BANNER);
  bjdevice->bjfltInkType =
      GetCurrentnValue(uidb.lpdbTop, uidb.dbsize, CNCL_CARTRIDGE);
  bjdevice->bjfltGrayScale =
      GetCurrentnValue(uidb.lpdbTop, uidb.dbsize, CNCL_GRAYSCALE);

  bjdevice->bjfltMarginType =
      GetCurrentnValue(uidb.lpdbTop, uidb.dbsize, CNCL_MARGINTYPE);
  if (bjdevice->bjfltMarginType == -1)
    bjdevice->bjfltMarginType = CND_MARGIN_NORMAL;

  bjdevice->bjfltDuplex =
      GetCurrentnValue(uidb.lpdbTop, uidb.dbsize, CNCL_DUPLEX_PRINTING);
  if (bjdevice->bjfltDuplex == -1)
    bjdevice->bjfltDuplex = CND_DUPLEX_OFF; /* for models not support duplex */

  bjdevice->bjfltInkcartridgesettings =
      GetCurrentnValue(uidb.lpdbTop, uidb.dbsize, CNCL_INKCARTRIDGESETTINGS);
  if (bjdevice->bjfltInkcartridgesettings == -1)
    bjdevice->bjfltInkcartridgesettings =
        CND_CARTRIDGE_NA; /* for models not support inkcartridgesettings */
...
void MakeModelnameConfname(char *argv0, char *modelname, char *confname,
  ...
  /* confname = /usr/lib/bjlib/cifip4200.conf etc.. */
  snprintf(tmpconfilename, sizeof(tmpconfilename), "%s%s", path,
           small_modelname);
...
short CheckSettings(LPBJFLTDEVICE bjdevice, char *confname) {
  ...
  /* Retrieve Name Information */
  if (CNCL_GetDBName(&uidb.nominfo, (void *)&bjlibdir, bjdevice->bjfltModelID) <
      0) {
    fprintf(stderr, "Error: Internal Database Name Error\n");
    goto onError;
  }
  ...
  if (bjdevice->bjfltInkcartridgesettings == CND_CARTRIDGE_BK) {
    if (formattype >= CND_FORMATTYPE_10) {
      if (SetCurrentFlag(uidb.lpdbTop, uidb.dbsize, CNCL_INKCARTRIDGESETTINGS,
                         bjdevice->bjfltInkcartridgesettings) < 0) {
        fprintf(stderr, "Error: INTERNAL ERROR(%d)\n", __LINE__);
        goto onError;
      }

      if (SetCurrentFlag(uidb.lpdbTop, uidb.dbsize, CNCL_MEDIATYPE,
                         bjdevice->bjfltMediaType) < 0) {
        fprintf(stderr, "Error: INTERNAL ERROR(%d)\n", __LINE__);
        goto onError;
      }

      if (SetCurrentFlag(uidb.lpdbTop, uidb.dbsize, CNCL_MARGINTYPE,
                         bjdevice->bjfltMarginType) < 0) {
        fprintf(stderr, "Error: INTERNAL ERROR(%d)\n", __LINE__);
        goto onError;
      }

      short result = CNCL_ConfirmCartrigeMediaMinus(
          &uidb.nominfo, (void *)bjlibdir, uidb.lpdbTop, uidb.dbsize);
      if (result == 0) {
        // not supports BK (changes to default value)
        bjdevice->bjfltInkcartridgesettings = GetDefaultnValue(
            uidb.lpdbTop, uidb.dbsize, CNCL_INKCARTRIDGESETTINGS);
      } else if (result == 1) {
        // supports BK
      } else {
        fprintf(stderr, "Error: INTERNAL ERROR(%d)\n", __LINE__);
      ...
      if ((bjdevice->bjfltMediaType != CND_MEDIA_PLAIN &&
           bjdevice->bjfltMediaType != CND_MEDIA_POSTCARD &&
           bjdevice->bjfltMediaType != CND_MEDIA_ADDRESS_POSTCARD &&
           bjdevice->bjfltMediaType != CND_MEDIA_ENVELOPE) ||
          (bjdevice->bjfltMarginType == CND_MARGIN_MINUS)) {
        bjdevice->bjfltInkcartridgesettings = GetDefaultnValue(
            uidb.lpdbTop, uidb.dbsize, CNCL_INKCARTRIDGESETTINGS);
      }
  ...
  if (bjdevice->bjfltModelID >= CND_MG6300) {
    if (bjdevice->bjfltMediaType == CND_MEDIA_OTHER_FINE_ART_PAPER) {
      if ((bjdevice->bjfltPaperSize != CND_SIZE_A4_ART_35_2) &&
          (bjdevice->bjfltPaperSize != CND_SIZE_LETTER_ART_35_2)) {
        fprintf(
            stderr,
            "Error: You cannot print with following combination\n\tmedia: %s\n",
            bjf_get_resource_string(confname, OPTSTRMEDIA,
                                    bjdevice->bjfltMediaType));
        fprintf(stderr, "\tpapersize: %s\n",
                bjf_get_resource_string(confname, OPTSTRPSIZE,
                                        bjdevice->bjfltPaperSize));
        goto onError;
      }
    }
  ...
  /* to Ver.3.70 */
    ...
    if ((bjdevice->bjfltMediaType == CND_MEDIA_FINE_ART_PAPER) ||
        (bjdevice->bjfltMediaType == CND_MEDIA_OTHER_FINE_ART_PAPER)) {
      if ((bjdevice->bjfltPaperSize != CND_SIZE_A4_FINE_ART) &&
          (bjdevice->bjfltPaperSize != CND_SIZE_LETTER_FINE_ART)) {
        fprintf(
            stderr,
            "Error: You cannot print with following combination\n\tmedia: %s\n",
            bjf_get_resource_string(confname, OPTSTRMEDIA,
    ...
    if ((bjdevice->bjfltPaperSize == CND_SIZE_A4_FINE_ART) ||
      ...
      if ((bjdevice->bjfltMediaType != CND_MEDIA_FINE_ART_PAPER) &&
          (bjdevice->bjfltMediaType != CND_MEDIA_OTHER_FINE_ART_PAPER)) {
        fprintf(stderr,
                "Error: You cannot print with following "
                "combination\n\tpapersize: %s\n",
                bjf_get_resource_string(confname, OPTSTRPSIZE,
                                        bjdevice->bjfltPaperSize));
...
static short ParseBbox(char *bbox, LPBJF_OPTINFO lpbjf_optinfo) {
  ...
  do {
    ...
    if ((lpbjf_optinfo->bbox.value[i]) < 0)
      return result;
